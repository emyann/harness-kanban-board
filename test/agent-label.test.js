...
function seedChain(store) {
  store.addIssue(kbIssue({ number: 41, status: 'ready', agent: 'claude', kb: { paths: ['src/a.js'] } }));
  store.addIssue(kbIssue({ number: 42, status: 'todo', agent: 'claude', blockedBy: [41], kb: { paths: ['src/b.js'] } }));
  store.addIssue(kbIssue({ number: 12, status: 'todo', agent: 'claude', blockedBy: [42], kb: { paths: ['docs/'] } }));
}
...
test('adopt onto another profile replaces the label instead of stacking a second one', async (t) => {
  const h = harness(t);
  h.store.addIssue(kbIssue({ number: 7, status: 'triage', agent: 'claude' }));

  assert.equal(await main(['adopt', '7', '--agent', 'codex', '--status', 'todo']), 0);

  assert.deepEqual(agentLabels(h.store, 7), ['kb:agent:codex']);
  assert.equal(agentOf(h.store.labelsOf(7)), 'codex', 'the card resolves to what adopt said it adopted it as');
  assert.equal(h.store.statusOf(7), 'todo');
...
test('the worked example: adopting the root as claude-track makes it a claimable track', async (t) => {
  ...
  assert.deepEqual(agentLabels(h.store, 12), ['kb:agent:claude-track'], 'no manual label surgery left to do');
...
test('adopt repairs a card that already carries two profile labels', async (t) => {
  ...
  h.store.addIssue(kbIssue({ number: 12, status: 'todo', agent: 'claude-track', labels: ['kb:agent:claude'] }));
  assert.equal(agentOf(h.store.labelsOf(12)), 'claude', 'precondition: the stale label is the one that wins');

  await main(['adopt', '12', '--agent', 'claude-track', '--status', 'todo']);

  assert.deepEqual(agentLabels(h.store, 12), ['kb:agent:claude-track']);
});

test('adopt keeps the labels of the board it is on, and re-adopting the same profile writes nothing', async (t) => {
  const h = harness(t);
  h.store.addIssue(kbIssue({ number: 7, status: 'todo', agent: 'claude', labels: ['bug'] }));
  await main(['adopt', '7', '--agent', 'claude', '--status', 'todo']);
  const before = h.store.revisionOf(7);

  await main(['adopt', '7', '--agent', 'claude', '--status', 'todo']);

  assert.deepEqual(h.store.labelsOf(7).sort(), ['bug', 'kb:agent:claude', 'kb:board:default', 'kb:status:todo']);
  assert.equal(h.store.revisionOf(7), before, 'nothing to change is nothing to write');
});

test('setAgent adds the label to a card that has none — a task adopted onto a bare issue', async (t) => {
  const h = harness(t);
  h.store.addIssue(kbIssue({ number: 9, status: 'triage', agent: null }));

  await main(['adopt', '9', '--agent', 'claude']);

  assert.deepEqual(agentLabels(h.store, 9), ['kb:agent:claude']);
...
test('doctor names every card on two profiles, which one it actually runs as, and the fix', async (t) => {
  ...
  h.store.addIssue(kbIssue({ number: 50, status: 'todo', agent: 'codex', labels: ['kb:agent:claude'] }));
  const results = [];
...
test('doctor is quiet on a clean board, and says how much it looked at', async (t) => {
  const h = harness(t);
  h.store.addIssue(kbIssue({ number: 41, status: 'ready', agent: 'claude' }));
  h.store.addIssue(kbIssue({ number: 12, status: 'todo', agent: 'claude-track' }));
  const results = [];
  const sink = { ok: (name, detail) => results.push({ name, ok: true, detail }), warn: (name, detail, fix) => results.push({ name, ok: null, detail, fix }) };
