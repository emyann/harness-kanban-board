...
different road (`src/liveness.ts:5-16`, `src/controller.ts:431-436`).
...
(`src/controller.ts:256-275`). A lapsed lease whose holder is a live local pid is
...
`reclaimed` event (`src/controller.ts:282-307`).
...
row — `Lease.acquiredAt` (`src/controller.ts:272`) and `Controller.startedAt`
...
(`src/controller.ts:428-430`, `src/liveness.ts:28-33`). An unparseable holder is
...
minutes (`src/controller.ts:434,153-155`). The invariant is **the lease outlives
the run**.
...
(`src/controller.ts:431-436`). The grace is sized off measured teardown (an 8s
timeout observed ending at ~10s) plus the record writes, so it is margin rather
than a guess (`src/controller.ts:156-158`).

`ControllerDeps.leaseMs` overrides the derivation wholesale
(`src/controller.ts:100,433`); no shipped path sets it — the daemon's call site
...
into reconcile (`src/daemon.ts:388`, `src/controller.ts:114-121,436`), and logs
that it did.
...
(`src/controller.ts:439`) and the pid check is its only guard, which covers every
...
| Duration | `timeoutMs + 5min` (`src/controller.ts:436`) | `max(3 × intervalMs, 90s)` (`src/daemon.ts:63-64`) |
