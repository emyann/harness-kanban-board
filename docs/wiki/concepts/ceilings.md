...
when it has nothing in flight to wait for (`src/controller.ts:588-593`). A refusal a pass caused
...
operator (`src/controller.ts:548-551`).
...
when it changes, and one narrated line silently undid that dedup (`src/controller.ts:596-608`). The
CLI renders it as one `refused: …` line (`src/hkb.ts:850`).
...
makes visible (`src/controller.ts:553-586`). The same reasoning shapes `resolveSpec`, whose failing
...
(`src/hkb.ts:1128-1129`; `prisma/schema.prisma:113-114`). Lowering `dailyBudgetUsd` mid-flight does not
...
through the commonest Job there is (`src/controller.ts:583-585`, `src/limits.ts:56-60`). The comment
...
(`src/controller.ts:556-559`). While one Job ran at a time that was harmless. The moment two could
...
(`src/controller.ts:570-573`) — the cap each live attempt was **claimed under**, written onto the
Attempt row in the same breath as the claim and from the same resolved spec the gate was just judged
against (`src/controller.ts:657-663`). It is `Float`, never null (`prisma/schema.prisma:491`), which
...
and there the retry genuinely buys something (`src/hkb.ts:922-944`).
...
(`src/controller.ts:560-564`). Whether a holder is really gone is the lease question, not this one —
...
Attempts enter the window by `startedAt` (`src/controller.ts:558`).
...
|---|---|---|---|
| stopped | `pausedAt`/`pausedBy` | `hkb stop` / `hkb start` (`src/hkb.ts:1336-1363`) | `` `hkb start` to resume `` (`src/limits.ts:69`) |
| concurrency | `maxConcurrent`, default 1 | `hkb boards set <slug> --max-concurrent <n>` (`src/hkb.ts:1360-1366`) | raise it, or wait for a run to finish (`src/limits.ts:77-78`) |
| budget | `dailyBudgetUsd`, null = no ceiling | `hkb boards set <slug> --daily-budget <usd>\|none` (`src/hkb.ts:1367-1375`) | raise it, or wait for a run or the window (`src/limits.ts:87-97`) |

Two edges worth knowing. `--max-concurrent 0` **drains** a board without stopping it — a distinct
state from the kill switch, and a deliberate one (`prisma/schema.prisma:127`, `src/hkb.ts:1362-1365`).
And `--daily-budget` accepts the literal `none` to clear the ceiling, because "no ceiling" and "a
ceiling of zero" are different configurations (`src/hkb.ts:1369-1373`).
...
board's default, then the built-in $1 (`src/spec.ts:63-67`, `src/spec.ts:161`).
...
- `hkb up --status` prints, per board, `STOPPED …` if the kill switch is set, then
  `limits  <n> of <m> slots running, $X of $Y spent in 24h` (`src/hkb.ts:1208-1217`). Those are the
...
- "The daemon is up but nothing is claimed" is answered by that line before it is answered anywhere
  else — a stopped board with a healthy daemon otherwise reads as fine (`src/hkb.ts:1206-1210`).
