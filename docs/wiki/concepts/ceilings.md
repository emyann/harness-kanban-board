...
- **`stopped`** — the board's kill switch is set (`Board.pausedAt`/`pausedBy`,
  `prisma/schema.prisma:97-100`). No amount of waiting un-stops it; only a person does.
...
when it has nothing in flight to wait for (`src/controller.ts:550-555`). A refusal a pass caused
itself is not news to report; it is a reason to wait for a slot (`src/controller.ts:72-74`). A pass
that reported *"2 of 2 slots in use"* while both slots were its own would end early and blame the
operator (`src/controller.ts:510-513`).
...
when it changes, and one narrated line silently undid that dedup (`src/controller.ts:558-570`). The
CLI renders it as one `refused: …` line (`src/hkb.ts:822`).
...
makes visible (`src/controller.ts:515-548`). The same reasoning shapes `resolveSpec`, whose failing
case is also silent (`src/spec.ts:16-20`), and `nextPhase` (`src/controller.ts:160-164`).

## Checked before a claim, never during a run

All three are checked before a claim and never mid-run (`src/limits.ts:9-10`,
`prisma/schema.prisma:102-103`). The reason is asymmetric cost: a ceiling that could stop a running
...
(`src/hkb.ts:1128-1129`; `prisma/schema.prisma:97-98`). Lowering `dailyBudgetUsd` mid-flight does not
reach into a running attempt either — the cap was handed to the runtime at spawn and nothing re-reads
it (`prisma/schema.prisma:399-402`). The ceiling binds admissions, not executions.

(Shutdown *is* different, and it is not a ceiling: `hkb down` aborts runs through an `AbortSignal`,
because a stop that took thirty minutes to return would not be a stop — `src/controller.ts:123-127`.)
...
through the commonest Job there is (`src/controller.ts:545-547`, `src/limits.ts:56-60`). The comment
...
(`src/controller.ts:518-521`). While one Job ran at a time that was harmless. The moment two could
...
finish (`src/limits.ts:39-43`, `src/controller.ts:75-77`).
...
(`src/controller.ts:532-535`) — the cap each live attempt was **claimed under**, written onto the
Attempt row in the same breath as the claim and from the same resolved spec the gate was just judged
against (`src/controller.ts:619-625`). It is `Float`, never null (`prisma/schema.prisma:424`), which
is precisely the difference from `Job.maxBudgetUsd`, which is `Float?` (`prisma/schema.prisma:195`) —
...
Kubernetes parallel, is the doc comment on the column (`prisma/schema.prisma:379-422`); distilled:
...
- Kubernetes puts it one table over: admission stamps a Pod with the limits a LimitRange supplied,
  ...
  not rewrite what is running (`prisma/schema.prisma:407-412`). An Attempt is this system's Pod.

Only the budget is frozen. `maxTurns`, `model` and `effort` are read once by the run itself and
nothing outside it asks (`prisma/schema.prisma:418-422`).
...
and there the retry genuinely buys something (`src/hkb.ts:890-912`).
...
(`src/controller.ts:522-526`). Whether a holder is really gone is the lease question, not this one —
...
(`prisma/schema.prisma:104`). There is no midnight, no reset, and no locale in the ceiling. Spend
...
Attempts enter the window by `startedAt` (`src/controller.ts:520`).

## Where each ceiling is set, and by whom

All three live on the `Board` — the namespace — because they are already per-repo facts
(`prisma/schema.prisma:88-91`), and all three are the operator's to set:

| Ceiling | Column | Operator command | In the refusal |
|---|---|---|---|
| stopped | `pausedAt`/`pausedBy` | `hkb stop` / `hkb start` (`src/hkb.ts:1109-1135`) | `` `hkb start` to resume `` (`src/limits.ts:69`) |
| concurrency | `maxConcurrent`, default 1 | `hkb boards set <slug> --max-concurrent <n>` (`src/hkb.ts:1314-1320`) | raise it, or wait for a run to finish (`src/limits.ts:77-78`) |
| budget | `dailyBudgetUsd`, null = no ceiling | `hkb boards set <slug> --daily-budget <usd>\|none` (`src/hkb.ts:1321-1329`) | raise it, or wait for a run or the window (`src/limits.ts:87-97`) |

Two edges worth knowing. `--max-concurrent 0` **drains** a board without stopping it — a distinct
state from the kill switch, and a deliberate one (`prisma/schema.prisma:111`, `src/hkb.ts:1316-1319`).
And `--daily-budget` accepts the literal `none` to clear the ceiling, because "no ceiling" and "a
ceiling of zero" are different configurations (`src/hkb.ts:1323-1327`).

`--max-budget` on `hkb boards set` is **not** a ceiling despite the neighbouring flags: it writes
`defaultMaxBudgetUsd`, a default a Job may override (`src/hkb.ts:1287`,
`prisma/schema.prisma:114-119`). The per-Job cap resolves three deep — the Job's own value, the
board's default, then the built-in $1 (`src/spec.ts:63-67`, `src/spec.ts:151`).
...
as a transient fault — it fails the Job even with retries left (`src/controller.ts:244-247`).
...
by the board's daily ceiling — $4.07 for nothing (`src/controller.ts:228-243`). Raising a cap is a
...
`hkb retry <id> --max-budget <usd>` that raises it (`src/controller.ts:177-187`). It stays
`resumable`, which is what keeps `lastSessionId` so that the deliberate retry continues rather than
starting cold (`src/controller.ts:248-251`).

## For ops

- `hkb up --status` prints, per board, `STOPPED …` if the kill switch is set, then
  `limits  <n> of <m> slots running, $X of $Y spent in 24h` (`src/hkb.ts:1162-1171`). Those are the
...
- "The daemon is up but nothing is claimed" is answered by that line before it is answered anywhere
  else — a stopped board with a healthy daemon otherwise reads as fine (`src/hkb.ts:1160-1164`).
