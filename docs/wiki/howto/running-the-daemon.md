...
process (`src/hkb.ts:1015-1035`), so a supervisor owns the lifecycle: it starts the
...
(`src/daemon.ts:195-196`, `src/hkb.ts:1031-1033`). Run `hkb boards add <slug> --repo
...
- **`Type=simple`** — `--foreground` never forks or writes a pid file. The
  process systemd starts is the process that runs the loop (`src/hkb.ts:1015`, `src/hkb.ts:1031-1034`).
- **No `ExecStop`.** SIGTERM is already the clean stop, and it is a *stop*, not a
  ...
  (`src/hkb.ts:1020-1029`). The loop then unwinds, records `daemon_down` and
...
- **`Restart=on-failure`, not `always`.** A tick that throws is caught and logged
  ...
  something structural. A clean SIGTERM exits 0 (`src/hkb.ts:1034`), and
...
--status` prints it for you when anything is running (`src/hkb.ts:1007-1009`):
...
`hkb up --status` exits 1 when no board is being served (`src/hkb.ts:1011`), so it
...
(`src/worktree.ts:698`). A systemd user unit does not inherit the `SSH_AUTH_SOCK`
...
(`src/daemon.ts:184-185`, `src/hkb.ts:1001-1005`):
