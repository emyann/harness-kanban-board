...
- [Where a hook command may say hkb is](./features/hook-install-shapes.md): A `matcher:"*"` hook that cannot resolve breaks every tool call in a repo — so what `hkb init` writes depends on where the running hkb lives, and only one that lives inside the repo earns the tracked settings file.
