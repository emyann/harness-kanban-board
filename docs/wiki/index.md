...
- [Starting and stopping a board (`hkb up` / `hkb down`)](./features/up-and-down.md): The two long-running processes as one idempotent verb — pid files as the whole protocol, why the child gets no KB_*, and the line between reporting exit 4 and supervising it.
