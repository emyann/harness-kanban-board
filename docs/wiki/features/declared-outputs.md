...
attempt** (`prisma/schema.prisma:265-270`, `src/controller.ts:1069-1144`). That single rule
...
the run went fine, and can still overturn it (`src/controller.ts:1236-1237`).
...
it (`src/controller.ts:1357-1368`). And absence becomes legible — a Job that produced
...
- **Export** — a repository-relative path the run wrote inside its checkout, copied out
  into `Board.repoPath` before the worktree is torn down (`src/worktree.ts:481-515`). The
...
declared)`, `src/worktree.ts:481`), so it looks like a call-site change. It is the wrong
...
other than the CLI (`src/worktree.ts:488`).
...
- An **export path** is the only one that has to police anything, and it polices a lot:
  ...
  (`src/worktree.ts:422-451`). The asymmetry is the point: an export names somewhere inside
...
something (`src/controller.ts:844-867`). Those paths go into the prompt as **absolute**
...
otherwise succeeded, and the holder kept its lease to the end (`src/controller.ts:1078-1084`,
...
half a failed run's output mixed into the repository (`src/worktree.ts:476-513`).
...
row rather than by the file surviving (`src/controller.ts:1119-1121`,
...
`src/controller.ts:1141-1144`). Nothing removes a non-empty one at all, which is why the
...
(`src/controller.ts:1236-1237`, `:1099`). It outranks the gate: a run that did not produce
what it promised has nothing worth approving (`src/controller.ts:1225-1235`). Only the
...
(`src/controller.ts:1108-1110`, `:1016-1018`). The messages are written for the person who
...
which case it gets the sandbox note instead (`src/controller.ts:988-989`,
