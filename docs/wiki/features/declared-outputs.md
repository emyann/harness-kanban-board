...
attempt** (`prisma/schema.prisma:265-270`, `src/controller.ts:1054-1129`). That single rule
...
the run went fine, and can still overturn it (`src/controller.ts:1221-1222`).
...
it (`src/controller.ts:1342-1353`). And absence becomes legible — a Job that produced
...
- **Export** — a repository-relative path the run wrote inside its checkout, copied out
  into `Board.repoPath` before the worktree is torn down (`src/worktree.ts:410-444`). The
...
declared)`, `src/worktree.ts:410`), so it looks like a call-site change. It is the wrong
...
other than the CLI (`src/worktree.ts:417`).
...
- An **export path** is the only one that has to police anything, and it polices a lot:
  ...
  (`src/worktree.ts:351-380`). The asymmetry is the point: an export names somewhere inside
...
something (`src/controller.ts:829-852`). Those paths go into the prompt as **absolute**
...
otherwise succeeded, and the holder kept its lease to the end (`src/controller.ts:1063-1069`,
...
half a failed run's output mixed into the repository (`src/worktree.ts:405-442`).
...
row rather than by the file surviving (`src/controller.ts:1104-1106`,
...
`src/controller.ts:1126-1129`). Nothing removes a non-empty one at all, which is why the
...
(`src/controller.ts:1221-1222`, `:1099`). It outranks the gate: a run that did not produce
what it promised has nothing worth approving (`src/controller.ts:1210-1220`). Only the
...
(`src/controller.ts:1093-1095`, `:1016-1018`). The messages are written for the person who
...
which case it gets the sandbox note instead (`src/controller.ts:973-974`,
