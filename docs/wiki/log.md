...
## 2026-08-28 — added features/hook-install-shapes for #146 (a new page, per "new feature → new page": `npm i -D hkb-cli` is a third install shape, and the first whose hook command is exact and portable at once, so `hkb init` writes `f="$CLAUDE_PROJECT_DIR/node_modules/hkb-cli/bin/hkb.js"; [ -f "$f" ] || exit 0; exec node "$f" hook <verb>` into the *tracked* `.claude/settings.json` with no `--shared-hooks`. Records what is expensive to rediscover: the `matcher:"*"` blast radius that made the tracked file off-limits in #85 and what changed; why `hkbOnPath` had to stop trusting an npx-augmented PATH and detect the install by path comparison instead; why the command guards its own file (a worker's worktree has no node_modules until `npm ci`; #144 is what closes that); doctor's two shape-specific verdicts; and the two pure functions that generalised rather than gaining a special case — `hookCommandNeeds` expanding a `VAR=` assignment, `isHkbHookCommand` accepting a `;`. `related` added on architecture/overview, which was NOT re-stamped: it is stale for #124/#127/#131 drift this task did not verify, and none of its claims are about where a hook command comes from)
