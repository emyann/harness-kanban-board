...
- **Store** — the board's state behind one interface (`openStore`, `src/store/index.js`; the names are `docs/local-first.md` §6.4). Two drivers: the **local** one (`src/store/local.js`, the default for a new board — the `kb-board` branch plus the `.git/hkb/index.db` index, composed) and the **GitHub** one (`src/store/github.js`, which `src/tasks.js` and `src/lock.js` re-export). `storeKind(ctx)` picks on one input: `store` in `.kanban/board.json` (absent means `github`); a `kb-board` ref in the checkout is deliberately not consulted (*architecture/store-seam*, *architecture/local-store*, *decisions/adr-006-local-store*).
