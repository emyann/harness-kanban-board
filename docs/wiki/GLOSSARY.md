...
- **Phase** — a Job's observed lifecycle state: `pending`, `running`, `succeeded`, `failed`, `suspended` (`prisma/schema.prisma`). `suspended` exists for the workloads that block on a human — a state no runtime can report.
