...
- **Store** — the board's state behind one interface (`openStore`, `src/store/index.js`; the names are `docs/local-first.md` §6.4). Two drivers: the **local** one (`src/store/local.js`, the default for a new board — the `kb-board` branch plus the `.git/hkb/index.db` index, composed) and the **GitHub** one (`src/store/github.js`, which `src/tasks.js` and `src/lock.js` re-export). `storeKind(ctx)` picks: `store` in `.kanban/board.json`, else "is there a `kb-board` branch" (*architecture/store-seam*, *architecture/local-store*, *decisions/adr-006-local-store*).
