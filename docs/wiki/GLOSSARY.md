...
- **Admission control** — enforcing an invariant at the tool boundary rather than asking for it in a prompt: a `canUseTool` gate that denies a spawn or mutates its input, e.g. injecting `isolation: "worktree"` (`src/admission.ts`). Named after the Kubernetes admission controller, and adopted after a prompt-level instruction was measurably ignored (*concepts/admission-control*).
