...
- **worker** — any harness holding one attempt on one task: Claude Code, Copilot CLI, Codex, a harness you wrote a `launch` array for, or the operator running the verbs by hand.
...
exits 4, and only a **supervisor** (cron, systemd, launchd) or the operator starts a fresh one. A worker never sees it.
