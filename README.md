...
- **A claim is a row.** `.git/hkb/index.db` (SQLite, `node:sqlite`, no dependency) holds the claims, the open
  ...
  one compare-and-swap `UPDATE`. It is host-local and rebuilt from the board ref whenever that ref has moved, so
...
- **Sync is git.** [`hkb sync`](#sharing-a-board) pushes `refs/kb/boards/<name>` to the remote and fast-forwards
  from it. The board ref has **one writing host**; the remote copy is the backup and every other clone reads it.
...
`kb:board:<slug>` issues are still there before it creates the board's ref, and refuses with the count and
the command above if they are. A forge it cannot reach refuses too, in its own sentence: creating the ref is
...
| Hermes | hkb |
|---|---|
| SQLite row | `cards/<n>.json` on the board's ref: title, body, status, agent, priority, paths, `blocked_by`, and a `<!-- kb: {...} -->` block for the task's own settings |
