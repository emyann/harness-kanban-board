...
export async function main(argv: string[]): Promise<number> {
  ...
  switch (verb) {
    ...
    case 'retry': {
      ...
      // The cap this Job actually ran under, which is not `job.maxBudgetUsd`: that column is null
      // whenever the Job took the board's default or the built-in, and reading it raw here threw
      // `Cannot read properties of null` on the single commonest path — a Job filed with no
      // `--max-budget` that then spends its cap. Everything below compares against the resolved
      // number, and the raise is still written to the Job, where it outranks the board next time.
      const ranUnder = resolveSpec(job, job.board).maxBudgetUsd.value;
      ...
      if (job.attempts[0]?.outcome === 'max_budget' && !(budget !== undefined && budget > ranUnder)) {
        throw usage(
          ...
          + `\`kb retry ${id} --max-budget ${(ranUnder * 2).toFixed(2)}\`, or file a smaller brief.`,
        );
      }
      ...
      emit(out, {
        id, phase: 'pending', maxBudgetUsd: budget ?? ranUnder, resume: job.lastSessionId, ...raise,
      }, () => {
