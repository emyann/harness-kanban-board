...
export function admissionCallback(policy: AdmissionPolicy = {}) {
  ...
  return async (input: HookInput): Promise<HookJSONOutput> => {
    ...
    // What used to sit here: two refusals that kept hkb's `pre-push` hook on the path, by denying
    // `--no-verify` and any `core.hooksPath` reassignment in a `Bash` command.
    //
    // Both are gone with the hook (ADR-018). They were a string parser guarding a git hook that
    // guarded a push the core required, and #63 measured the layer under them failing anyway — the
    // hook never reached the worktree the harness cuts for a subagent. The core requires no push,
    // so there is nothing on the path to keep there.
    //
    // What refuses a push to a protected branch is the forge, centrally, which is where Kubernetes
    // puts admission: at the API server, not on the node.
